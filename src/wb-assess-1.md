...
Functions are named groups of code that can be reused which can help keep you from repeating yourself and folllows DRY principles.


**_What is the difference between `console.log` and `return`?_**

```
Console log will print what you tell it to in the console and is mainly used for debugging. Return is used for getting a specific value from a function.
```

**_What's the difference between defining and calling a function?_**

```
Defining is when you are setting the function paramaters and actions when called. Calling a function is when you're exucuting the code previously defined in the function.
```
...
**_String_**
A string is a data type representing a sequence of characters; it can consist of letters, numbers, symbols, or spaces.


**_Number_**

```
Represents numerical value data types.
```

**_Boolean_**

```
This is a logical value data type that is either True or False.
```

**_Array_**

```
An array is a list of data values of the same data type.
```
